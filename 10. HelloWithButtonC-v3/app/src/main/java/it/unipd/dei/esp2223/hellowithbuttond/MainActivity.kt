...
fun MainScreen(modifier: Modifier = Modifier)
{
    ...
    ConstraintLayout(modifier = modifier) {
        ...
        Button(
            ...
            // Set the action to be performed when the button is pressed
            onClick = {
                val i: Int = try { t.toInt() }
                             catch(exception: NumberFormatException) { -1 }
                // getString() not available,
                // stringResource() can be invoked only in a composable function
                t = "${i+1}"
            }
        ) {
